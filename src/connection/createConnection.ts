...
export function createConnection(localUserId: UserId, remoteUserId: UserId): Connection {
    ...
    async function createOffer(): Promise<RTCSessionDescription>{
        const offer = await peerConnection.createOffer(offerOptions)
        await peerConnection.setLocalDescription(new RTCSessionDescription(offer));
        return peerConnection.localDescription;
    }

    async function receiveOffer(description: RTCSessionDescriptionInit) : Promise<RTCSessionDescription>{
    ...
    async function iceCandidate(payload: RTCIceCandidate) {
    ...
    async function updateTracks(tracks: MediaStreamTrack[]) {
